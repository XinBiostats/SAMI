...
def list_files(directory, substring, extension):
    files = [file for file in os.listdir(directory) if substring in file and file.endswith(extension)]
    return files
...
def adata_filter(adata,abundance=0.1,prevalence=0.1):
    adata=add_preva_abund(adata)
    adata_filter = adata[adata.obs['abundance']>=abundance,adata.var['prevalence']>=prevalence]
    return adata_filter
