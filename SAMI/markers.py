...
class Markers:
    """
    A class for identifying and visualizing marker features in spatial omics data.
    ...
    """
    def __init__(self, data_path, result_path, region, modality):
    ...
    def findmarkers(self,adata,adj_pval_cutoff,top,adata2=None):
        """
        Identifies marker features in spatial omics data using Wilcoxon rank-sum test.

        Parameters
        ----------
        adata : AnnData
            AnnData object containing the main dataset.
        adj_pval_cutoff : float
            Threshold for adjusted p-values to identify significant markers.
        top : int
            Number of top-ranked markers to retain.
        ...
        -----
        Two CSV files:
        - `pvalue.csv` : Contains statistical test results for all features.
        - `marker.csv` : Contains top marker features based on the p-value and log2FC.
        """
        ...
        for i in range(len(cluster_list)):
            pvalue_temp=pd.DataFrame(columns=['feature','omics','pvalue','avg_log2FC','abs_avg_log2FC','pct1','pct2','mean_a','mean_b','adj_pvalue','cluster','rank'])
            if adata2 is not None:
                common_features = adata.var.index.intersection(adata2.var.index)
            else:
                common_features = adata.var.index
            ...
            for feature in common_features:
                ...
                if len(a)!=0 and len(b)!=0:
                    ...
                    if (pct1>0.1)&(pct2>0.1):
                        ...
                        pvalue_feature=pd.DataFrame({'feature':feature,'omics':omics,'pvalue':[pvalue],'avg_log2FC':avg_log2FC,'abs_avg_log2FC':abs_avg_log2FC,'pct1':pct1,'pct2':pct2,'mean_a':mean_a,'mean_b':mean_b,'cluster':cluster_list[i]})
        ...
        file_path = os.path.join(self.result_path,f'markers/{self.modality}/')
    ...
    def circular_tree(self,clusters=None,top=5, show=True):
        """
        Generates a circular tree visualization of marker features.

        Parameters
        ----------
        clusters : list, optional
            List of clusters to visualize. If None, all clusters are included.
        top : int, optional
            Number of top markers per cluster to include. Default is 5.
        show : bool, optional
            If True, displays the plot instead of saving. Default is True.

        Saves
        -----
        PNG file:
        - `circulartree_{region}_{clusters}.png` : Circular tree plot of markers.
        """        
        ...
        plt.savefig(os.path.join(self.result_path,f'markers/{self.modality}/circulartree_{self.region}_{cluster_str}.png'))
        if show==False:
            plt.close()
        else:
            plt.show()
    ...
    def volcano_plot(self,cluster,show=True):
        """
        Generates a volcano plot to visualize differential expression.

        Parameters
        ----------
        cluster : int
            Cluster for which the volcano plot is generated.
        show : bool, optional
            If True, displays the plot instead of saving. Default is True.
        ...
        -----
        PNG file:
        - `vol_{region}_{cluster}.png` : Volcano plot of log2(FC) vs -log10(p-value).
        """
        pvalues = pd.read_csv(os.path.join(self.result_path,f'markers/{self.modality}/{self.region}_pvalue.csv'))
        ...
        plt.savefig(os.path.join(self.result_path,f'markers/{self.modality}/vol_{self.region}_{cluster}.png'))
        if show == False:
            plt.close()
        else:
            plt.show()
    ...
    def density_plot(self,adata,cluster,marker,show=False):
        """
        Generates a density plot for the expression of a selected marker.
        ...
        -----
        PNG file:
        - `density_{region}_{cluster}_{marker}.png` : Density plot of marker expression.
        """
        adata= adata[:,marker]
        ...
        plt.savefig(os.path.join(self.result_path,f'markers/{self.modality}/density_{self.region}_{cluster}_{marker}.png'))
        if show == False:
            plt.close()
        else:
            plt.show()
