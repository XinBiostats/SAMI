...
class Clusters:
    ...
    def clustering(self):
        ...
        file_path = f'../results/clustering/{self.modality}'
    ...
    def plot_umap_cluster(self,size=50,show=False):
        adata = sc.read(f'../results/clustering/{self.modality}/{self.region}_{self.resolution}.h5ad')
        color='leiden'
...
class Cluster_Integration:
    def __init__(self,adata,adata_ref,modality):
        self.adata = adata
    ...
    def integrate(self):
        adata1 = sc.read(f'../results/clustering/{self.modality}/{self.adata}')
        adata2 = sc.read(f'../results/clustering/{self.modality}/{self.adata_ref}')
        ...
        adata1.write(f'../results/clustering/{self.modality}/{self.region1}_integrated.h5ad')
        adata2.write(f'../results/clustering/{self.modality}/{self.region2}_integrated.h5ad')
    ...
    def plot_umap_cluster(self,region,size=50,show=False):
        adata = sc.read(f'../results/clustering/{self.modality}/{region}_integrated.h5ad')
        color='leiden'
        clusters_colors = dict(zip([str(i) for i in range(102)], sc.pl.palettes.default_102))
        colorlist=adata.obs[color].cat.categories.tolist()
        n_cluster=str(len(colorlist))
    ...
    def plot_overlap_umap(self,show=False):
        adata1 = sc.read(f'../results/clustering/{self.modality}/{self.region1}_integrated.h5ad')
        adata2 = sc.read(f'../results/clustering/{self.modality}/{self.region2}_integrated.h5ad')
        ...
        plt.savefig(f'../results/clustering/{self.modality}/{self.region1}_{self.region2}_umap_overlap.png',dpi=200)
        if show==False:
            plt.close()
        else:
            plt.show()
